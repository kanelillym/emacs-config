...
I use ivy for search because David Wilson uses it in Emacs from Scratch. It's fine, I don't really know any advantages or disadvantages to different completion frameworks. But I live and die by =counsel-switch-buffer= so I can never change lmao
...
Before actually configuring org, create a few helper functions that can be called later.
